...
def main(input_file, output_file, min_support_rate, quiet, k):
    """
    可疑的根本原因微服务集挖掘和微服务排名

    :param input_file:异常调用检测的数据
    :param output_file:根据可疑评分排序的根源微服务列表
    :param min_support_rate:处于异常迹线状态的微服务集的支持阈值
    :param quiet:是否将结果打印到终端
    :param k:前k个可疑微服务集。
    :return:根据可疑评分排序的根本原因微服务列表
    """
...
class ItemsetHandler:
    ...
    def __init__(self, data: pd.DataFrame, **kwargs):
        ...
        self._traces_containing_item = {key: frozenset(value) for key, value in
                                        item2trace_id_sets.items()}  # 包含该微服务的trace集合
        ...
        self.p_a_given_b, self.frequent_patterns = self._frequent_pattern_on_abnormal(
            **kwargs)  # the support and frequent pattern
    ...
    def __call__(self, input_file: pd.DataFrame, enable_PRFL=False, **kwargs) -> List[str]:
        ...
        if not enable_PRFL:
            ...
            def jaccard_similarity(_itemset):
                ...
                ep = normal_score * (
                        len(itemset_handler.traces_containing_pattern(_itemset)) - len(
                    itemset_handler.abnormal_traces_containing_pattern(_itemset))
                )
        ...
        def in_out_diff(_item, _pattern, return_="diff") -> float:
            """
            它是由数字之间的差来计算的包含传入/输出异常调用的跟踪在微服务中所有包含可疑集合的痕迹中。
            """
            if len(_pattern) <= 0:
                return 0.
...
if __name__ == "__main__":
    ...
    input_file = rf'{dataset_path}\invo_anomaly_detection_2.pkl'
    output_file = rf'{dataset_path}\final_result_2.pkl'
    ...
    main(input_file=input_file, output_file=output_file,
         min_support_rate=min_support_rate, k=k, quiet=quiet)
